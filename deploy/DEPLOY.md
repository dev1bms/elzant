...
**6) تأكيد الحضور (RSVP) + زرّ التهنئة:** تظهر أزرار **«تأكيد الحضور / الاعتذار»** وزرّ **«كتابة التهنئة»** في صفحة الدعوة الخاصة (`/i/<token>/`). كل النصوص (السؤال، عناوين الأزرار، ردود الشكر) تُحرَّر من `/admin/ → إعدادات الزفاف → «تأكيد الحضور والتهنئة»` — بلا كود. يظهر ردّ كل مدعو في قائمة «المدعوون» (عمود/فلتر «الحضور»).

**7) أزرار RSVP داخل واتساب (اختياري):** إن أردت أن يؤكّد المدعو من داخل واتساب مباشرةً، أنشئ Content Template من نوع **Quick reply** بزرّين، واضبط الـ **payload id** لكلٍّ منهما على `rsvp_yes` و`rsvp_no`. ثم في Twilio Console اضبط **Incoming Message webhook** للمُرسِل على `https://elzant.com/webhooks/twilio/inbound/`. يتحقّق النظام من التوقيع، يطابق المدعو برقمه، يسجّل الردّ، ويرسل شكراً لبقاً تلقائياً. (إن لم تضبط payload، يُطابَق نصّ الزرّ مع عناوين الأزرار في الإعدادات.)
